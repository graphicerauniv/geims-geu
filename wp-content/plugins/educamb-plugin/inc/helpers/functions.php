...
function get_instructors_categories() {
	$options = array();
	$taxonomy = 'instructors_cat';
	if (!empty($taxonomy)) {
		$terms = get_terms(
				array(
					'parent' => 0,
...
function educamb_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('educamb_redux_custom_fonts_load', 'educamb_redux_custom_fonts_load');
function educamb_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = educamb_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/educamb/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [educamb_social_share_output description]
 *
 ...
 */
function educamb_social_share_output( $icon, $color = false ) {
...
function educamb_get_social_icons()
{
	...
	if( educamb_set( $options, 'social_media' ) && is_array( educamb_set( $options, 'social_media' ) ) )
	{
		$total = count( educamb_set( $options, 'social_media' ) ) - 2;
		foreach( educamb_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = educamb_set( $social_icon, 'title');
			$class = educamb_set( $social_icon, 'class');
			$link = educamb_set( $social_icon, 'social_link');
			$icon = educamb_set( $social_icon, 'social_icon');
...
add_action('educamb_social_share_output_two', 'educamb_social_share_output_two');
...
<ul class="social-links pull-right clearfix">
	<li><h5><?php esc_html_e('Share :', 'educamb'); ?></h5></li>
...
function educamb_get_social_icon()
{
