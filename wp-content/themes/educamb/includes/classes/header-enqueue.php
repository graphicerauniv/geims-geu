...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'educamb' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
		);

		$scripts = apply_filters( 'EDUCAMB/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( EDUCAMB_NONCE ),
		);

		wp_localize_script( 'jquery', 'educamb_data', $header_data );

		if ( educamb_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', educamb_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'EDUCAMB/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "educamb-{$name}", $style );
			} else {
				wp_enqueue_style( "educamb-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = educamb_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= educamb_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'educamb-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'educamb-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$data = \EDUCAMB\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'educamb' . '_options-mods' );

		if ( $custom_font = educamb_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('educamb_redux_custom_fonts_load', $custom_font );
