...
class WXRImporter extends \WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'sevenbite' ) );
		}
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning( sprintf(
							__( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'sevenbite' ),
							$this->version,
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'sevenbite' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'sevenbite' ) );
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'sevenbite' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s', 'sevenbite' ),
				$data['post_title'],
				$data['post_type']
			) );
			return false;
		...
		if ( $post_exists ) {
			$this->logger->info( sprintf(
				__( '%s "%s" already exists.', 'sevenbite' ),
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice( sprintf(
					__( 'Skipping attachment "%s", fetching attachments disabled', 'sevenbite' ),
					$data['post_title']
				) );
				return false;
			}
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import "%s" (%s)', 'sevenbite' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported "%s" (%s)', 'sevenbite' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %d remapped to %d', 'sevenbite' ),
			$original_id,
			$post_id
		) );
					...
					if ( 'post_format' === $taxonomy ) {
						...
						if ( empty( $term_id ) ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								$this->logger->warning( sprintf(
									esc_html__( 'Failed to import term: %s - %s', 'sevenbite' ),
									esc_html( $taxonomy ),
									esc_html( $term['name'] )
								) );
								continue;
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'sevenbite' ) );
		}
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import user "%s"', 'sevenbite' ),
				$userdata['user_login']
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported user "%s"', 'sevenbite' ),
			$userdata['user_login']
		) );
		$this->logger->debug( sprintf(
			__( 'User %d remapped to %d', 'sevenbite' ),
			$original_id,
			$user_id
		) );
	...
	protected function process_term( $data, $meta ) {
		...
		$this->logger->debug( sprintf(
			__( 'Term %d remapped to %d', 'sevenbite' ),
			$original_id,
			$term_id
		) );
	...
	protected function process_term_meta( $meta, $term_id, $term ) {
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( is_wp_error( $result ) ) {
					$this->logger->warning( sprintf(
						__( 'Failed to add metakey: %s, metavalue: %s to term_id: %d', 'sevenbite' ),
						$key,
						$value,
						$term_id
					) );
					...
					$this->logger->debug( sprintf(
						__( 'Meta for term_id %d : %s => %s ; successfully added!', 'sevenbite' ),
						$term_id,
						$key,
						$value
					) );
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'sevenbite' ),
					$code,
		...
		// 	unlink( $upload['file'] );
		// 	return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'sevenbite' ) );
		// }

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'sevenbite' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'sevenbite' ), size_format( $max_size ) );
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug( sprintf(
				...
				__( 'Running post-processing for post %d', 'sevenbite' ),
				$post_id
			) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the post parent for "%s" (post #%d)', 'sevenbite' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with parent %d, but could not be found', 'sevenbite' ),
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for "%s" (post #%d)', 'sevenbite' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with author "%s", but could not be found', 'sevenbite' ),
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				$this->logger->debug( sprintf(
					__( 'Post %d was marked for post-processing, but none was required.', 'sevenbite' ),
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update "%s" (post #%d) with mapped data', 'sevenbite' ),
					get_the_title( $post_id ),
					$post_id
				) );
				$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning( sprintf(
				__( 'Could not find the menu object for "%s" (post #%d)', 'sevenbite' ),
				get_the_title( $post_id ),
				$post_id
			) );
			$this->logger->debug( sprintf(
				__( 'Post %d was imported with object "%d" of type "%s", but could not be found', 'sevenbite' ),
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the comment parent for comment #%d', 'sevenbite' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with parent %d, but could not be found', 'sevenbite' ),
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for comment #%d', 'sevenbite' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with author %d, but could not be found', 'sevenbite' ),
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update comment #%d with mapped data', 'sevenbite' ),
					$comment_id
				) );
				continue;
			}
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if( empty( $termid ) || ! is_numeric( $termid ) ) {
				$this->logger->warning( sprintf(
					__( 'Faulty term_id provided in terms-to-be-remapped array %s', 'sevenbite' ),
					$termid
					) );
				continue;
			}
			...
			if( empty( $term_taxonomy ) ){
				$this->logger->warning( sprintf(
					__( 'No taxonomy provided in terms-to-be-remapped array for term #%d', 'sevenbite' ),
					$term_id
					) );
				continue;
			}
			...
			if ( empty( $parent_slug ) ) {
				$this->logger->warning( sprintf(
					__( 'No parent_slug identified in remapping-array for term: %d', 'sevenbite' ),
					$term_id
					) );
				continue;
			}
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) || ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				$this->logger->warning( sprintf(
					__( 'The term(%d)"s parent_slug (%s) is not found in the remapping-array.', 'sevenbite' ),
					$term_id,
					$parent_slug
					) );
				continue;
			...
			if ( empty( $termattributes ) ) {
				$this->logger->warning( sprintf(
					__( 'No data returned by get_term_by for term_id #%d', 'sevenbite' ),
					$term_id
					) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
			$this->logger->warning( sprintf(
					__( 'Could not update "%s" (term #%d) with mapped data', 'sevenbite' ),
					$termattributes['name'],
					$term_id
				) );
				$this->logger->debug( $result->get_error_message() );
			...
			delete_term_meta( $term_id, '_wxr_import_parent' );
			$this->logger->debug( sprintf(
				__( 'Term %d was successfully updated with parent %d', 'sevenbite' ),
				$term_id,
				$mapped_parent
			) );
		}
	...
	function remap_featured_images() {
		...
		$this->logger->info( esc_html__( 'Starting remapping of featured images', 'sevenbite' ) );
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->mapping['post'][ $value ] ) ) {
				...
				if ( $new_id !== $value ) {
					$this->logger->info( sprintf( esc_html__( 'Remapping featured image ID %d to new ID %d for post ID %d', 'sevenbite' ), $value, $new_id, $post_id ) );
